...
@Composable
fun TransactionInfoDialog(transaction: Transaction?, response: Resource<String>?, onConfirm: () -> Unit, onDismiss: (popUpTo: Boolean) -> Unit) {
    Card {
        Column(
            Modifier
                .padding(18.dp)
            ...
            when {
                ...
                response.isError() -> {
                    ...
                    Spacer(Modifier.height(20.dp))
                    Text(response.data ?: "", style = MaterialTheme.typography.body1)
                    Row(
                        Modifier
                            .fillMaxWidth(), horizontalArrangement = Arrangement.End) {
                        TextButton(onClick = { onDismiss(true) }) {
